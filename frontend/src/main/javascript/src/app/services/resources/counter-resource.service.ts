...
export class CounterResource {
  ...
  getAll(projectId: number): IPromise<any> {
    return this.$http.get(`${env.apiUrl}/projects/${projectId}/counters`)
      .then(response => (response.data as any[]).map(c => Counter.fromData(c)));
  }
  ...
  create(projectId: number, counter: Counter): IPromise<any> {
    return this.$http.post(`${env.apiUrl}/projects/${projectId}/counters`, counter)
  ...
  update(projectId: number, counter: Counter): IPromise<any> {
    return this.$http.put(`${env.apiUrl}/projects/${projectId}/counters/${counter.id}`, counter)
      .then(response => Counter.fromData(response.data));
  }
  ...
  remove(projectId: number, counter: Counter): IPromise<any> {
    return this.$http.delete(`${env.apiUrl}/projects/${projectId}/counters/${counter.id}`);
  }
  ...
  removeMany(projectId: number, counters: Counter[]): IPromise<any> {
    ...
    return this.$http.delete(`${env.apiUrl}/projects/${projectId}/counters/batch/${ids}`);
  }
}
