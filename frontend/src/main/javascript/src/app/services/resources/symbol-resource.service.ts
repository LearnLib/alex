...
export class SymbolResource {
  ...
  get(projectId: number, symbolId: number) {
    return this.$http.get(`${env.apiUrl}/projects/${projectId}/symbols/${symbolId}`)
  ...
  getAll(projectId: number, includeHiddenSymbols = false) {
    ...
    return this.$http.get(`${env.apiUrl}/projects/${projectId}/symbols${params}`)
  ...
  create(projectId: number, symbol: AlphabetSymbol) {
    return this.$http.post(`${env.apiUrl}/projects/${projectId}/symbols`, symbol)
      .then(response => new AlphabetSymbol(response.data));
  }
  ...
  createMany(projectId: number, symbols: AlphabetSymbol[]) {
    return this.$http.post(`${env.apiUrl}/projects/${projectId}/symbols/batch`, symbols)
      .then(response => (<any[]>response.data).map(s => new AlphabetSymbol(s)));
  }
  ...
  moveMany(symbols: AlphabetSymbol[], group: SymbolGroup) {
    ...
    return this.$http.put(`${env.apiUrl}/projects/${project}/symbols/batch/${ids}/moveTo/${group.id}`, {});
  }
  ...
  update(symbol: AlphabetSymbol) {
    return this.$http.put(`${env.apiUrl}/projects/${symbol.project}/symbols/${symbol.id}`, symbol)
      .then(response => new AlphabetSymbol(response.data));
  }
  ...
  remove(symbol: AlphabetSymbol) {
    return this.$http.post(`${env.apiUrl}/projects/${symbol.project}/symbols/${symbol.id}/hide`, {});
  }
  ...
  removeMany(symbols: AlphabetSymbol[]) {
    const ids = symbols.map(s => s.id).join(',');
    const project = symbols[0].project;
    return this.$http.post(`${env.apiUrl}/projects/${project}/symbols/batch/${ids}/hide`, {});
  }
  ...
  delete(symbol: AlphabetSymbol) {
    return this.$http.delete(`${env.apiUrl}/projects/${symbol.project}/symbols/${symbol.id}`);
  }
  ...
  deleteMany(projectId: number, symbols: AlphabetSymbol[]): IHttpPromise<any> {
    ...
    return this.$http.delete(`${env.apiUrl}/projects/${projectId}/symbols/batch/${symbolIds}`);
  }
  ...
  recover(symbol: AlphabetSymbol) {
    return this.$http.post(`${env.apiUrl}/projects/${symbol.project}/symbols/${symbol.id}/show`, {});
  }
  ...
  recoverMany(symbols: AlphabetSymbol[]) {
    const ids = symbols.map(s => s.id).join(',');
    const project = symbols[0].project;
    return this.$http.post(`${env.apiUrl}/projects/${project}/symbols/batch/${ids}/show`, {});
  }

  getUsages(projectId: number, symbolId: number) {
    return this.$http.get(`${env.apiUrl}/projects/${projectId}/symbols/${symbolId}/usages`)
      .then(res => SymbolUsageResult.fromData(res.data));
  }

  export(projectId: number, config: any): IPromise<any> {
    return this.$http.post(`${env.apiUrl}/projects/${projectId}/symbols/export`, config)
  }
}
