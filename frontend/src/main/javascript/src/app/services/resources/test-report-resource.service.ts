...
export class TestReportResource {
  ...
  getAll(projectId: number, page = 0, size = 25): IPromise<any> {
    return this.$http.get(`${env.apiUrl}/projects/${projectId}/tests/reports?page=${page}&size=${size}`)
  ...
  get(projectId: number, testReportId: number, format: any = null): IPromise<any> {
    return this.$http.get(`${env.apiUrl}/projects/${projectId}/tests/reports/${testReportId}`, {params: format})
      .then(response => response.data);
  }
  ...
  getLatest(projectId: number): IPromise<any> {
    return this.$http.get(`${env.apiUrl}/projects/${projectId}/tests/reports/latest`)
      .then(response => response.data);
  }
  ...
  remove(projectId: number, testReportId: number): IPromise<any> {
    return this.$http.delete(`${env.apiUrl}/projects/${projectId}/tests/reports/${testReportId}`);
  }
  ...
  removeMany(projectId: number, testReports: any[]): IPromise<any> {
    ...
    return this.$http.delete(`${env.apiUrl}/projects/${projectId}/tests/reports/batch/${ids}`);
  }
}
