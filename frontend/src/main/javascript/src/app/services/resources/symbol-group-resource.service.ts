...
export class SymbolGroupResource {
  ...
  getAll(projectId: number): IPromise<any> {
    return this.$http.get(`${env.apiUrl}/projects/${projectId}/groups`)
  ...
  create(projectId: number, group: SymbolGroup): IPromise<any> {
    return this.$http.post(`${env.apiUrl}/projects/${projectId}/groups`, group)
      .then(response => new SymbolGroup(response.data));
  }

  createMany(projectId: number, groups: SymbolGroup[]): IPromise<any> {
    return this.$http.post(`${env.apiUrl}/projects/${projectId}/groups/batch`, groups)
      .then(res => (<any[]>res.data).map(g => new SymbolGroup(g)));
  }

  importMany(projectId: number, groups: SymbolGroup[]): IPromise<any[]> {
    return this.$http.post(`${env.apiUrl}/projects/${projectId}/groups/import`, groups)
      .then(res => (<any[]>res.data).map(g => new SymbolGroup(g)));
  }
  ...
  update(group: SymbolGroup): IPromise<any> {
    return this.$http.put(`${env.apiUrl}/projects/${group.project}/groups/${group.id}`, group)
      .then(response => new SymbolGroup(response.data));
  }
  ...
  move(group: SymbolGroup): IPromise<any> {
    return this.$http.put(`${env.apiUrl}/projects/${group.project}/groups/${group.id}/move`, group)
      .then(response => new SymbolGroup(response.data));
  }
  ...
  remove(group: SymbolGroup): IPromise<any> {
    return this.$http.delete(`${env.apiUrl}/projects/${group.project}/groups/${group.id}`);
  }
}
