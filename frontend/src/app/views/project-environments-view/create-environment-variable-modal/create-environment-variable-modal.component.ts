...
@Component({
  selector: 'create-environment-variable-modal',
  templateUrl: './create-environment-variable-modal.component.html'
})
export class CreateEnvironmentVariableModalComponent {
  ...
  createVariable(): void {
    ...
    this.projectEnvironmentApi.createVariable(this.environment.project, this.environment.id, this.variable).subscribe(
      createdVariable => {
        this.modal.close(createdVariable);
      },
      res => {
        this.errorMessage = `The variable could not be created. ${res.error.message}`;
      }
    );
  }
}
